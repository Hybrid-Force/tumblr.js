...
TumblrClient.prototype = {
  ...
  _pickCredential: function() {
    if (Array.isArray(this.credentials)) {
      return this.credentials[Math.floor(Math.random() * this.credentials.length)];
    } else {
  ...
  _get: function (path, params, callback, addApiKey) {
    ...
    params = params || {};
    var credentials = this._pickCredential();
    if (addApiKey) {
